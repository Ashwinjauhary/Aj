...
const moreNav = [
    ...
    { name: "Uses", href: "/uses", icon: <Wrench size={16} /> },
    { name: "Now", href: "/now", icon: <Clock size={16} /> },
    { name: "Playground", href: "/playground", icon: <Gamepad2 size={16} /> },
    { name: "Terminal", href: "/terminal", icon: <Terminal size={16} /> },
    { name: "Changelog", href: "/changelog", icon: <List size={16} /> },
];
...
export default function FloatingDock() {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2">
            ...
            {/* Main dock */}
            <nav className="glass rounded-full p-2 flex items-center gap-1">
                {mainNav.map((item) => {
                    const isActive = pathname === item.href;
                    return (
                        <Link key={item.name} href={item.href} className="relative group">
                            <span className="sr-only">{item.name}</span>
                            <div className={`relative px-3 py-2.5 rounded-full flex items-center justify-center transition-colors duration-300 ${isActive ? "text-white" : "text-gray-400 hover:text-white"}`}>
                                ...
                                <span className="relative z-10 group-hover:scale-110 transition-transform duration-200 block">{item.icon}</span>
                ...
                {/* More button */}
                <button onClick={() => setShowMore(v => !v)} className="relative group px-3 py-2.5 rounded-full text-gray-400 hover:text-white transition-colors cursor-none">
